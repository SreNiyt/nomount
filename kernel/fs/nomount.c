...
DEFINE_HASHTABLE(nomount_rules_by_vpath,    NOMOUNT_HASH_BITS);
DEFINE_HASHTABLE(nomount_rules_by_real_ino, NOMOUNT_HASH_BITS);
DEFINE_HASHTABLE(nomount_rules_by_v_ino,    NOMOUNT_HASH_BITS);
DEFINE_HASHTABLE(nomount_basenames_ht,      NOMOUNT_HASH_BITS);
DEFINE_HASHTABLE(nomount_uid_ht,            NOMOUNT_UID_HASH_BITS);
LIST_HEAD(nomount_rules_list);
LIST_HEAD(nomount_private_dirs_list);
DEFINE_MUTEX(nomount_write_mutex);
...
static inline struct nomount_dir_node *__nomount_find_dir_rcu(unsigned long ino, dev_t dev) {
    struct nomount_dir_node *dir;
    hash_for_each_possible_rcu(nomount_dirs_ht, dir, node, ino) {
        if (dir->dir_ino == ino && dir->dir_dev == dev) return dir;
    }
    return NULL;
}

static inline bool __nomount_is_traversal_allowed_rcu(struct inode *inode) {
    return __nomount_find_dir_rcu(inode->i_ino, inode->i_sb->s_dev) != NULL;
}

static const char *nomount_build_path_from_pwd(const char *rel_name, size_t name_len, size_t *out_len, const char **out_path)
{
    struct path pwd;
    char *page_buf = (char *)__getname();
    char *cwd_str, *end_ptr;
    size_t dir_len;

    if (unlikely(!page_buf)) return NULL;
    ...
    if (unlikely(IS_ERR_OR_NULL(cwd_str))) {
        __putname(page_buf);
        return NULL;
    }
...
int nomount_handle_permission(struct inode *inode, int mask)
{
    ...
    if (__nomount_should_skip() || unlikely(!inode)) return 0;

    rcu_read_lock();
    hash_for_each_possible_rcu(nomount_rules_by_real_ino, rule, real_ino_node, inode->i_ino) {
        if (rule->real_ino == inode->i_ino && rule->real_dev == inode->i_sb->s_dev) {
            rcu_read_unlock();
            return (mask & (MAY_WRITE | MAY_APPEND)) ? 0 : 1;
        }
    }
    hash_for_each_possible_rcu(nomount_rules_by_v_ino, rule, v_ino_node, inode->i_ino) {
    ...
    if (static_branch_unlikely(&nomount_active_dirs)) {
        hash_for_each_possible_rcu(nomount_dirs_ht, dir, node, inode->i_ino) {
            if (dir->dir_ino == inode->i_ino && dir->dir_dev == inode->i_sb->s_dev) {
                rcu_read_unlock();
                if (mask & (MAY_READ | MAY_WRITE | MAY_APPEND)) return 0;
                return (mask & MAY_EXEC) ? 1 : 0;
            }
        }
    }
    rcu_read_unlock();
...
{
    struct nomount_rule *rule;
    const char *s, *last_slash, *check_name, *full_path;
    size_t name_len, b_len, full_len;
    u32 b_hash;
    ...
    s = name->name;
    if (unlikely(strstr(s, ".."))) return name;
    name_len = strlen(s);
    if (unlikely(name_len <= 1)) {
        if (name_len == 1 && s[0] == '/') return name;
        if (name_len == 0) return name;
    }
    ...
    rcu_read_lock();
    if (unlikely(!list_empty(&nomount_private_dirs_list) && s[0] == '/')) {
        if (unlikely(current_uid().val >= AID_APP_START)) {
            struct nomount_dir_node *priv_dir;
            list_for_each_entry_rcu(priv_dir, &nomount_private_dirs_list, private_list) {
                if (name_len >= priv_dir->dir_path_len && !memcmp(s, priv_dir->dir_path, priv_dir->dir_path_len)) {
    ...
    hash_for_each_possible_rcu(nomount_basenames_ht, rule, basename_node, b_hash) {
        if (rule->b_len == b_len && !memcmp(rule->basename, check_name, b_len)) {
            if (unlikely(s[0] != '/')) {
                if (!page_buf) {
                    rcu_read_unlock();
                    page_buf = nomount_build_path_from_pwd(s, name_len, &full_len, &full_path);
                    if (unlikely(!page_buf)) return name;
                    rcu_read_lock();
                    hash_for_each_possible_rcu(nomount_basenames_ht, rule, basename_node, b_hash) {
                        if (rule->b_len == b_len && !memcmp(rule->basename, check_name, b_len)) {
    ...
    list_for_each_entry_rcu(rule, &nomount_rules_list, list) {
        if ((rule->flags & NM_FLAG_IS_DIR) && full_len > rule->vp_len) {
            if (!memcmp(full_path, rule->virtual_path, rule->vp_len) && full_path[rule->vp_len] == '/') {
                ...
                if (likely(rule->rp_len + suffix_len < PATH_MAX - 64)) {
...
found:
    if (likely(rule->rp_len < PATH_MAX)) {
        memcpy((char *)name->name, rule->real_path, rule->rp_len + 1);
        nm_debug("Redirected: %s -> %s\n", full_path, rule->real_path);
    }
    rcu_read_unlock();
    if (unlikely(page_buf)) __putname(page_buf);
    return name;
...
int nomount_handle_iterate_dir(struct file *file, struct dir_context *ctx)
{
    ...
    if (likely(res >= 0) && (ctx->pos == old_pos || ctx->pos >= nomount_magic_pos)) {
        rcu_read_lock();
        curr_dir = __nomount_find_dir_rcu(d_backing_inode(file->f_path.dentry)->i_ino, d_backing_inode(file->f_path.dentry)->i_sb->s_dev);

        if (curr_dir) {
            array = rcu_dereference(curr_dir->child_array);
            if (array) {
                if (ctx->pos < nomount_magic_pos) ctx->pos = nomount_magic_pos;
                v_index = (unsigned long)(ctx->pos - nomount_magic_pos);

                for (i = (u32)v_index; i < array->num_children; i++) {
                    struct nomount_child_name *child = array->entries[i];
                    if (unlikely(!dir_emit(ctx, child->name, child->name_len, child->fake_ino, child->d_type)))
                        break;
                    ctx->pos++;
                }
            }
...
bool nomount_spoof_mmap_metadata(struct inode *inode, dev_t *dev, unsigned long *ino)
{
    ...
    if (unlikely(!inode || !dev || !ino || __nomount_should_skip())) return false;

    rcu_read_lock();
    rule = nomount_get_rule_by_inode(inode);
    if (likely(rule)) {
...
static inline struct nomount_dir_node* __nomount_get_or_create_dir(unsigned long ino, dev_t dev)
{
    ...
    lockdep_assert_held(&nomount_write_mutex);

    hash_for_each_possible_rcu(nomount_dirs_ht, curr, node, ino) {
...
static int __nomount_collect_parents(struct nomount_rule *rule, struct dentry *d)
{
    struct dentry *parent;
    struct inode *inode;
    char *r_tmp = rule->real_path, *slash;
    char *slashes[64];
    int p_count = 0, err = 0;

    dget(d);
    while (d && !IS_ROOT(d) && p_count < 64) {
        inode = d_backing_inode(d);
        if (unlikely(!inode)) break;

        if (p_count > 0) {
            if (unlikely(rule->flags & NM_FLAG_SRC_PRIVATE)) {
                struct nomount_dir_node *dir_node;

                dir_node = __nomount_get_or_create_dir(inode->i_ino, inode->i_sb->s_dev);
                if (unlikely(!dir_node)) {
                    err = -ENOMEM;
                    break;
                }
                if (!dir_node->is_private) {
                    dir_node->is_private = true;
                    dir_node->dir_path = kstrdup(r_tmp, GFP_KERNEL);
                    if (unlikely(!dir_node->dir_path)) {
                        err = -ENOMEM;
                        break;
                    }
                    dir_node->dir_path_len = (u16)strlen(dir_node->dir_path);
...
static int __nomount_inject_child_locked(struct nomount_dir_node *dir_node, struct nomount_rule *rule,
                                          ...
                                          unsigned char type, unsigned long child_fake_ino)
{
    ...
    struct nomount_child_name *new_entry;
    u32 i, old_num = 0;

    if (unlikely(!dir_node)) return -EINVAL;
    if (unlikely(name_len > 255)) return -ENAMETOOLONG;
    rule->parent_ino = dir_node->dir_ino;
    rule->parent_dev = dir_node->dir_dev;

    old_array = rcu_dereference_protected(dir_node->child_array, mutex_is_locked(&nomount_write_mutex));
    if (old_array) {
    ...
    new_entry = kmalloc(sizeof(struct nomount_child_name) + name_len + 1, GFP_KERNEL);
    if (unlikely(!new_entry)) return -ENOMEM;

    new_entry->fake_ino = child_fake_ino;
    new_entry->name_len = (u16)name_len;
    ...
    new_array->entries[old_num] = new_entry;
...
{
    struct nm_child_array *old_array, *new_array;
    struct nomount_child_name *deleted_entry = NULL;
    int found_idx = -1;
    u32 i, num, dst = 0;
    ...
    for (i = 0; i < num; i++) {
        if (old_array->entries[i]->fake_ino == fake_ino) {
            found_idx = i;
            deleted_entry = old_array->entries[i];
            break;
        }
    }
    ...
    if (num == 1) {
        ...
        for (i = 0; i < num; i++) {
            ...
            new_array->entries[dst++] = old_array->entries[i];
        }
        rcu_assign_pointer(dir_node->child_array, new_array);
...
{
    struct nomount_rule *ex, *irule = NULL, *t_rule, *pending_rules[64];
    struct path p_path, r_path_struct;
    char *v_tmp = rule->virtual_path, *r_tmp = rule->real_path;
    char *slash_v, *slash_r, *b_slash, *slashes_v[64], *slashes_r[64];
    ...
    while (p_count < 64) {
        ...
        if (!slash_v || slash_v == v_tmp) {
            if (likely(kern_path("/", LOOKUP_FOLLOW, &p_path) == 0)) {
                ...
                {
                    struct nomount_dir_node *dir = __nomount_get_or_create_dir(current_parent_ino, current_parent_dev);
                    if (unlikely(!dir)) err = -ENOMEM;
                    else err = __nomount_inject_child_locked(dir, t_rule, child_name, child_name_len, child_name_hash,
...
static int __nomount_add_rule(const char *v_path, const char *r_path, u16 v_len, u16 r_len, u32 flags)
{
    ...
    if (r_path_dentry) {
        err = __nomount_collect_parents(rule, r_path_dentry);
        if (unlikely(err)) {
            mutex_unlock(&nomount_write_mutex);
            dput(r_path_dentry);
            kfree(rule->virtual_path);
                               ...
                               struct hlist_head *d_victims)
{
    ...
    hash_for_each_possible(nomount_rules_by_vpath, rule, vpath_node, hash) {
        if (rule->v_hash == hash && rule->vp_len == v_len &&
            ...
            hash_for_each_possible_rcu(nomount_dirs_ht, dir, node, rule->parent_ino) {
...
static int nomount_genl_add_rule(struct sk_buff *skb, struct genl_info *info)
{
    if (info->attrs[NOMOUNT_ATTR_PAYLOAD]) {
        ...
        char *v_buf = (char *)__getname();
        char *r_buf = (char *)__getname();
        int len = nla_len(attr);
        int pos = 0, err = 0;

        if (unlikely(!v_buf || !r_buf)) {
        ...
        char *v_str = (char *)nla_data(info->attrs[NOMOUNT_ATTR_VIRTUAL_PATH]);
        char *r_str = (char *)nla_data(info->attrs[NOMOUNT_ATTR_REAL_PATH]);
...
static int nomount_genl_del_rule(struct sk_buff *skb, struct genl_info *info)
{
    ...
    if (info->attrs[NOMOUNT_ATTR_PAYLOAD]) {
        ...
        char *v_path = (char *)nla_data(info->attrs[NOMOUNT_ATTR_VIRTUAL_PATH]);
        mutex_lock(&nomount_write_mutex);
